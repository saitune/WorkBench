...
clock_t start_time;
int Count[NumOfDecomposition];
int RecCall = 0;


void print(const char dudeney[DudeneySize],const char hs[NumOfHamilton][N])
{
  for(int i=0;i<DudeneySize;i++)for(int j=0;j<N;j++)printf("%d",hs[dudeney[i]][j]);
  printf("\n");
}



void print(const int array[][N],int size)
{
...
//ハミルトンサイクル生成
void make_hamilton(int lv,int& hn,char h[N],bool vflg[N],char hs[][N])
{
  if(lv==N){
    if(h[1]<h[N-1]){
...
//ビットフィールド生成
void make_twopath_bf(const char hs[][N],uint64_t Twopath_bf[][SizeOfTwopath],const int f[N][N][N])
{
...
void make_onepath_bf(const char hs[][N],uint64_t Onepath_bf[][SizeOfOnepath],const int g[][N])
{
  for(int i=0;i<NumOfHamilton;i++)for(int j=0;j<N;j++){
    const int l=hs[i][j];
    const int m=hs[i][(j+1)%N];
...
//hs,bfを最少ビットごとに分類
void classify_by_bit(const char hs[][N],const uint64_t Twopath_bf[][SizeOfTwopath],const uint64_t Onepath_bf[][SizeOfOnepath],int class_size[],int classified_hs[NumOfTwopath][NumOfHamilton][N],
                   uint64_t classified_Twopath_bf[][NumOfHamilton][SizeOfTwopath],uint64_t classified_Onepath_bf[][NumOfHamilton][SizeOfOnepath])
{
  for (int i = 0; i < NumOfHamilton; i++){//ハミルトンサイクルすべてを探索
    ...
    for (int j = 0; j < N; j++)classified_hs[min_bit][class_size[min_bit]][j] = hs[i][j];
    for (int j = 0;j < SizeOfTwopath; j++)classified_Twopath_bf[min_bit][class_size[min_bit]][j] = Twopath_bf[i][j];
    for (int j = 0;j < SizeOfOnepath; j++)classified_Onepath_bf[min_bit][class_size[min_bit]][j] = Onepath_bf[i][j];
    class_size[min_bit]++;
  }
}
...
//ビット照合(互いに素かどうか)
bool test_twopath_bit(const uint64_t bf_a[SizeOfTwopath],const uint64_t bf_b[SizeOfTwopath])
{
  for(int i=0;i<SizeOfTwopath;i++)if((bf_a[i]&bf_b[i])!=0)return false;
  return true;
}

bool test_onepath_bit(const uint64_t bf_a[SizeOfOnepath],const uint64_t bf_b[SizeOfOnepath])
{
...
//ビットセット
void set_twopath_bit(uint64_t check_Twopath_bf[SizeOfTwopath],const uint64_t Twopath_bf[SizeOfTwopath])
{
...
void set_onepath_bit(uint64_t check_Onepath_bf[SizeOfOnepath],const uint64_t Onepath_bf[SizeOfOnepath])
{
...
//ビットリセット
void reset_twopath_bit(uint64_t check_Twopath_bf[SizeOfTwopath],const uint64_t Twopath_bf[SizeOfTwopath])
{
  for (int i = 0; i < SizeOfTwopath; i++)check_Twopath_bf[i] &= ~Twopath_bf[i];
}
...
void reset_onepath_bit(uint64_t check_Onepath_bf[SizeOfOnepath],const uint64_t Onepath_bf[SizeOfOnepath])
{
  for (int i = 0; i < SizeOfOnepath; i++)check_Onepath_bf[i] &= ~Onepath_bf[i];
}
...
//同形判定
bool is_normal(const int num_of_hd,const int dudeney[][N],int trns[][N],int& trns_size)
{
  //メモ実装前
#if 0
  //ベクター生成
...
next2:;
  ...
  //ベクター生成
  std::vector<std::vector<std::vector<int>>> h(num_of_hd+1,std::vector<std::vector<int>>(SizeOfDecomposition,std::vector<int>(N,0)));
  ...
  for (int i = 0; i < trns_size; i++){
    ...
    //射影
    for (int j = 0; j < hn; j++)for (int k = 0; k < N; k++)temp_for_h[j][k] = trns[i][dudeney[j][k]];
    
    //標準化
    for (int j = 0; j < hn; j++)for (int k = 0; k < N; k++)normalize(temp_for_h[j],&h[j / SizeOfDecomposition][j % SizeOfDecomposition][0]);
...
//check_twopathに含まれないビットのみで構成されるハミルトンサイクルとそれに対応するビットフィールドを1
//newClassfied_hs,newClassified_Onepath_bf,newClassified_Twopath_bfに格納
void extract(const int class_size[],const int classified_hs[][NumOfHamilton][N],const uint64_t classified_Onepath_bf[][NumOfHamilton][SizeOfOnepath],
             const uint64_t classified_Twopath_bf[][NumOfHamilton][SizeOfTwopath],const uint64_t check_twopath_bf[],
             int newClassified_hs[][NumOfHamilton][N],uint64_t newClassified_Onepath_bf[][NumOfHamilton][SizeOfOnepath],
             uint64_t newClassified_Twopath_bf[][NumOfHamilton][SizeOfTwopath],int newClass_size[])
{
  for (int i = 0; i < NumOfOnepath; i++)for (int j = 0; j < class_size[i]; j++)if (test_twopath_bit(check_twopath_bf,classified_Twopath_bf[i][j])){
    for (int k = 0; k < N; k++)newClassified_hs[i][newClass_size[i]][k] = classified_hs[i][j][k];
    for (int k = 0; k < SizeOfOnepath; k++)newClassified_Onepath_bf[i][newClass_size[i]][k] = classified_Onepath_bf[i][j][k];
    for (int k = 0; k < SizeOfTwopath; k++)newClassified_Twopath_bf[i][newClass_size[i]][k] = classified_Twopath_bf[i][j][k];
    newClass_size[i]++;
  }
}
...
//デュードニー集合生成
void make_dudeney(int hn,int dudeney[DudeneySize][N],uint64_t check_Twopath_bf[SizeOfTwopath],uint64_t check_Onepath_bf[SizeOfOnepath],int class_size[],
                 int classified_hs[][NumOfHamilton][N],uint64_t classified_Twopath_bf[][NumOfHamilton][SizeOfTwopath],uint64_t classified_Onepath_bf[][NumOfHamilton][SizeOfOnepath],
                 int trns[][N],int trns_size)
{
  ...
  for(int i=SizeOfOnepath-1;0<=i;i--)std::cout << static_cast<std::bitset<64> >(check_Onepath_bf[i]);
  printf("\n");
  printf("check_twopath_bf\n");
  for(int i=SizeOfTwopath-1;0<=i;i--)std::cout << static_cast<std::bitset<64> >(check_Twopath_bf[i]);
  printf("\n");
  printf("min_bit = %d\n",min_bit);
  getchar();
  getchar();
  ...
  for (int i = 0; i < class_size[min_bit]; i++){
    ...
    //decompositionの最初のサイクルが1つ前のdecompositionの最初のサイクルより小さかったら枝刈り
    if (hn != 0 && hn % SizeOfDecomposition == 0 && cmp(dudeney[hn - SizeOfDecomposition],classified_hs[min_bit][i]) == 1)continue;
    ...
    if (test_onepath_bit(check_Onepath_bf,classified_Onepath_bf[min_bit][i]) 
      && test_twopath_bit(check_Twopath_bf,classified_Twopath_bf[min_bit][i])
        ...
        if (hn % SizeOfDecomposition == SizeOfDecomposition - 1){
          ...
          //同形判定
          if (!is_normal(hn / SizeOfDecomposition,dudeney,trns,new_trns_size))goto next;
          ...
          /*
          //set newClassfied_hs
          int newClassified_hs[NumOfOnepath][NumOfHamilton][N];
          std::fill_n(newClassified_hs[0][0],NumOfOnepath * NumOfHamilton * N,0);

          //set newClassified_Onepath_bf
          uint64_t newClassified_Onepath_bf[NumOfOnepath][NumOfHamilton][SizeOfOnepath];
          std::fill_n(newClassified_Onepath_bf[0][0],NumOfOnepath * NumOfHamilton * SizeOfOnepath,0);

          //set newClassified_Twopath_bf
          uint64_t newClassified_Twopath_bf[NumOfOnepath][NumOfHamilton][SizeOfTwopath];
          std::fill_n(newClassified_Twopath_bf[0][0],NumOfOnepath * NumOfHamilton * SizeOfTwopath,0);
          //set newClass_size
          int newClass_size[NumOfOnepath];
          std::fill_n(newClass_size,NumOfOnepath,0);

          //printf("extract\n");
          extract(class_size,classified_hs,classified_Onepath_bf,classified_Twopath_bf,check_Twopath_bf,
            newClassified_hs,newClassified_Onepath_bf,newClassified_Twopath_bf,newClass_size);
          */
          //printf("finish extract\n");

          //make_dudeney(hn + 1,dudeney,check_Twopath_bf,NewCheck_Onepath_bf,newClass_size,newClassified_hs,newClassified_Twopath_bf,newClassified_Onepath_bf,trns,new_trns_size);
          make_dudeney(hn + 1,dudeney,check_Twopath_bf,NewCheck_Onepath_bf,class_size,classified_hs,classified_Twopath_bf,classified_Onepath_bf,trns,new_trns_size);
        }
          ...
          //make_dudeney(hn + 1,dudeney,check_Twopath_bf,check_Onepath_bf,class_size,classified_hs,classified_Twopath_bf,classified_Onepath_bf,trns,trns_size);
          make_dudeney(hn + 1,dudeney,check_Twopath_bf,check_Onepath_bf,class_size,classified_hs,classified_Twopath_bf,classified_Onepath_bf,trns,trns_size);
...
int main(void)
{
  ...
  classify_by_bit(hs,Twopath_bf,Onepath_bf,class_size,classified_hs,classified_Twopath_bf,classified_Onepath_bf);
#if 0
  ...
  */
  ...
  //extractをかける
  extract(class_size,classified_hs,classified_Onepath_bf,classified_Twopath_bf,check_Twopath_bf,newClassified_hs,newClassified_Onepath_bf,newClassified_Twopath_bf,newClass_size);
  ...
  make_dudeney(1,dudeney,check_Twopath_bf,check_Onepath_bf,newClass_size,newClassified_hs,newClassified_Twopath_bf,newClassified_Onepath_bf,trns,0);
#if 0
  printf("makeDudeney finished\n");
#endif
  for (int i = 0; i < NumOfDecomposition; i++)printf("HD %d => %d\n",i + 1,Count[i]);
